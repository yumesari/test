...
function checkExtra(aryMessage, event) {
	...
	// 追加チェックの追加処理
	onCheckExtraArray(aryMessage, event);
}

/**
 * 追加チェック追加処理(配列)。
 */
function onCheckExtraArray(aryMessage, event) {
	// 追加チェック追加処理(配列)長を取得
	var aryLength = aryCheckExtra.length;
	// 追加チェック追加処理(配列)毎に処理
	for (var i = 0; i < aryLength; i++) {
		// 追加チェック追加処理を取得
		var onLoadFunction = aryCheckExtra[i];
		// 追加チェック追加処理を実施
		onLoadFunction(aryMessage, event);
	}
}

//アドオン用チェック関数呼び出し
checkExtra.prototype.checkExtraAddon(aryMessage, event);

// アドオン用チェック関数
function checkExtraAddon(aryMessage, event) {
	// 未処理
}
